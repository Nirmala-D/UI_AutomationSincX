...
public class TC_004_KnowledgeSets extends BaseClass {
  @Test
  public void tc_004_Verify_EDIT_Course_Status_disabled() throws Exception {
		...
		driver.findElement(
				By.xpath("(//table[@class='MuiTable-root _nzyk8q']//td//i[@class='material-icons-outlined'])[1]"))
				.click();
		// click on Adriele test
		...
		WebElement dropdownButton = driver.findElement(By.xpath(
				"//button[@class='MuiButtonBase-root MuiIconButton-root MuiAutocomplete-popupIndicator _1hhulki']"));
		...
		for (WebElement option : options) {
			...
			if (actual.equalsIgnoreCase("Disabled")) {
				option.click();
				optionFound = true;
				System.out.println("Disabled option selected"); // Debugging line to confirm selection
				break;
			}
		}

		if (!optionFound) {
			System.out.println("Disabled option not found"); // Debugging line if the option was not found
		}
